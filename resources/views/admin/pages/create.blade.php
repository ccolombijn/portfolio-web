...
@section('content')
    ...
    <form action="{{ route('admin.pages.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full p-2 border border-stone-300 rounded">
            @error('title')<p class="text-red-500 text-sm">{{ $message }}</p>@enderror
        </div>
        <div class="mb-4">
            <label for="title" class="block mb-2">Title</label>
            <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full p-2 border border-stone-300 rounded">
            @error('title')<p class="text-red-500 text-sm">{{ $message }}</p>@enderror
        </div>

        <div class="mb-4">
            <label for="route" class="block mb-2">Route</label>
            <input type="text" name="route" id="route" value="{{ old('route') }}" class="w-full p-2 border border-stone-300 rounded">
            @error('route')<p class="text-red-500 text-sm">{{ $message }}</p>@enderror
        </div>

        <div class="mb-4">
            <label for="route" class="b-2">Exclude in nav</label>
        ...
        <div class="mb-4 w-1/3">
            <label for="route" class="block mb-2">View</label>
